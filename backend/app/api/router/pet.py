...
async def get_pet_info(pet_id: int, db: Session = Depends(get_session)):
    try:
        return db_pet_to_pet_dto(await crud.get_pet(pet_id, db))
    except Exception:
        raise fastapi.HTTPException(status_code=404, detail=f"Pet with id {pet_id} not found")
...
async def update_pet(
    ...
    try:
        ...
        raise fastapi.HTTPException(status_code=400, detail=f"Pet update error: {str(e)}")
...
async def update_pet_donation_flag(
    ...
    try:
        ...
        pet_update = PetUpdate(
            able_to_donate=able_to_donate
        )
        db_pet = await crud.update_pet(current_user.user_id, pet_id, pet_update, db)
        return db_pet_to_pet_dto(db_pet)
    except Exception as e:
        raise fastapi.HTTPException(status_code=400, detail=f"Pet update donation ability error: {str(e)}")
