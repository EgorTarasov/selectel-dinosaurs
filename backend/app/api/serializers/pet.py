...
def db_pet_to_pet_dto(db_pet: Pet, is_full=True):
    if is_full:
        ...
        last_donation = (
            donations[-1].date if donations else datetime(year=1970, month=1, day=1)
        )
        days_left_after_last_donation = (datetime.now() - last_donation).days
        cooldown_donation_days = max(
            0, COOLDOWN_DAYS_AFTER_DONATION - days_left_after_last_donation
        )
