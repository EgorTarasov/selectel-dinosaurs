...
class BloodRequest(Base, TimestampMixin):
    ...
    blood_request_response = relationship(
        "BloodRequestResponse", back_populates="blood_request"
    )
...
class BloodRequestResponse(Base, TimestampMixin):
    ...
    blood_request_id: Mapped[int] = mapped_column(
        sa.Integer, sa.ForeignKey("blood_requests.id", ondelete="CASCADE"), index=True
    )
    msg: Mapped[str] = mapped_column(sa.String, nullable=False)
    amount: Mapped[int] = mapped_column(sa.Integer, nullable=False)
    pet_id: Mapped[int] = mapped_column(
        sa.Integer, sa.ForeignKey("pets.id", ondelete="CASCADE"), index=True
    )

    blood_request: Mapped[BloodRequest] = relationship(
        "BloodRequest", back_populates="blood_request_response"
    )
